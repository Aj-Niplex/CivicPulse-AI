...
export const AdminDashboard: React.FC = () => {
  const { issues, subscribeToIssues, updateIssueStatus } = useIssueStore();
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
          ...
          <div className="flex gap-2">
            <input type="text" placeholder="Search issues..." className="px-3 py-1.5 text-sm border border-gray-300 rounded-md outline-none focus:border-primary" />
          </div>
        </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {issue.status === 'Reported' && (
                        <Button size="sm" onClick={() => handleUpdateStatus(issue.issueId, 'Assigned')}>Accept AI Plan</Button>
                      )}
                    </div>
                  </td>
                </tr>
